...
cat .aidlc/ROADMAP.md 2>/dev/null
```
...
```bash
ls .aidlc/phases/{target}-*/*-SUMMARY.md 2>/dev/null
```
...
2. Find phase directory: `.aidlc/phases/{target}-{slug}/`
...
This will:
- Delete: .aidlc/phases/{target}-{slug}/
...
```bash
if [ -d ".aidlc/phases/{target}-{slug}" ]; then
  rm -rf ".aidlc/phases/{target}-{slug}"
  echo "Deleted: .aidlc/phases/{target}-{slug}/"
fi
```
...
mv ".aidlc/phases/18-dashboard" ".aidlc/phases/17-dashboard"
```
...
grep -r "Phase 18" .aidlc/phases/17-*/ 2>/dev/null
grep -r "Phase 19" .aidlc/phases/18-*/ 2>/dev/null
# etc.
```
...
Changes:
- Deleted: .aidlc/phases/{target}-{slug}/
