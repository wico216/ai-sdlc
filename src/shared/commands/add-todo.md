...
<objective>
Capture an idea, task, or issue that surfaces during a AI-SDLC session as a structured todo for later work.
...
```bash
mkdir -p .aidlc/todos/pending .aidlc/todos/done
```
</step>

<step name="check_existing_areas">
```bash
ls .aidlc/todos/pending/*.md 2>/dev/null | xargs -I {} grep "^area:" {} 2>/dev/null | cut -d' ' -f2 | sort -u
```
...
```bash
grep -l -i "[key words from title]" .aidlc/todos/pending/*.md 2>/dev/null
```
...
Write to `.aidlc/todos/pending/${date_prefix}-${slug}.md`:
...
<step name="confirm">
```
Todo saved: .aidlc/todos/pending/[filename]
