...
Todos are captured during work sessions with __CMD_PREFIX__add-todo.
...
- `__CMD_PREFIX__check-todos` → show all
- `__CMD_PREFIX__check-todos api` → filter to area:api only
</step>
...
- `__CMD_PREFIX__check-todos [area]` to filter by area
- `q` to exit
```
