...
```bash
ls .aidlc/phases/${PHASE}-*/*-RESEARCH.md 2>/dev/null
ls .aidlc/phases/${PHASE}-*/*-PLAN.md 2>/dev/null
```
...
PHASE_DIR=$(ls -d .aidlc/phases/${PHASE}-* 2>/dev/null | head -1)
if [ -z "$PHASE_DIR" ]; then
  # Create phase directory from roadmap name
  PHASE_NAME=$(grep "Phase ${PHASE}:" .aidlc/ROADMAP.md | sed 's/.*Phase [0-9]*: //' | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
  mkdir -p ".aidlc/phases/${PHASE}-${PHASE_NAME}"
  PHASE_DIR=".aidlc/phases/${PHASE}-${PHASE_NAME}"
fi
...
### Spawn sdlc-phase-researcher

Gather additional context for research prompt:

```bash
# Get phase description from roadmap
PHASE_DESC=$(grep -A3 "Phase ${PHASE}:" .aidlc/ROADMAP.md)

# Get requirements if they exist
REQUIREMENTS=$(cat .aidlc/REQUIREMENTS.md 2>/dev/null | grep -A100 "## Requirements" | head -50)

# Get prior decisions from STATE.md
DECISIONS=$(grep -A20 "### Decisions Made" .aidlc/STATE.md 2>/dev/null)
...
ROADMAP_CONTENT=$(cat .aidlc/ROADMAP.md)

# Read optional files (empty string if missing)
REQUIREMENTS_CONTENT=$(cat .aidlc/REQUIREMENTS.md 2>/dev/null)
# CONTEXT_CONTENT already loaded in step 4
...
## 8. Spawn sdlc-planner Agent

Display stage banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 AI-SDLC ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
## 10. Spawn sdlc-plan-checker Agent
...
Spawn sdlc-planner with revision prompt:
...
- [ ] sdlc-phase-researcher spawned with CONTEXT.md (constrains research scope)
- [ ] Existing plans checked
- [ ] sdlc-planner spawned with context (CONTEXT.md + RESEARCH.md)
