...
Configuration options for `.aidlc/` directory behavior.
...
- Skip all `git add`/`git commit` for `.aidlc/` files
- User must add `.aidlc/` to `.gitignore`
...
**Auto-detection:** If `.aidlc/` is gitignored, `commit_docs` is automatically `false` regardless of config.json. This prevents git errors when users have `.aidlc/` in `.gitignore`.
...
- Direct path searches work: `rg "pattern" .aidlc/` finds files
- Broad searches skip gitignored: `rg "pattern"` skips `.aidlc/`

**When `search_gitignored: true`:**
- Add `--no-ignore` to broad rg searches that should include `.aidlc/`
- Only needed when searching entire repo and expecting `.aidlc/` matches

**Note:** Most AI-SDLC operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
3. **Existing tracked files:** If `.aidlc/` was previously tracked:
   ```bash
   git rm -r --cached .aidlc/
