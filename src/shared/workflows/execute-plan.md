...
**If file missing but .aidlc/ exists:**
...
**If .aidlc/ doesn't exist:** Error - project not initialized.
...
3. After Task tool returns with agent_id:
   ...
   a. Write agent_id to current-agent-id.txt:
      echo "[agent_id]" > .aidlc/current-agent-id.txt
...
if [ ! -f .aidlc/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .aidlc/agent-history.json
fi

# Clear any stale current-agent-id (from interrupted sessions)
# Will be populated when subagent spawns
rm -f .aidlc/current-agent-id.txt
```
...
if [ -f .aidlc/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .aidlc/current-agent-id.txt)
...
- Skip all git operations for .aidlc/ files
...
**If .aidlc/codebase/ doesn't exist:**
Skip this step.
</step>
