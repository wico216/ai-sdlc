...
1. **sdlc-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **sdlc-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
ls .aidlc/phases/${PADDED_PHASE}-*/*-CONTEXT.md .aidlc/phases/${PHASE}-*/*-CONTEXT.md 2>/dev/null
```
...
if [ -z "$PHASE_DIR" ]; then
  ...
  mkdir -p ".aidlc/phases/${PADDED_PHASE}-${PHASE_NAME}"
  PHASE_DIR=".aidlc/phases/${PADDED_PHASE}-${PHASE_NAME}"
fi
```
...
```
Created: .aidlc/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
